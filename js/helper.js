...
function create_custom_field(container){
    ...
    var field_label = blank.clone().attr({id : field_name, name : 'key['+ts+']', value : 'Nome do campo (editar)', onClick : 'this.select();'}).addClass('key');

    field_name = 'value_' + ts;
    var field_value = blank.clone().attr({id : field_name, name : 'value['+ts+']', value : '', onClick : 'this.select()'});
    ...
    var new_field_container_row = $('<div></div>').addClass('row-fluid each-field').append(new_field_container1).append(new_field_container2);
